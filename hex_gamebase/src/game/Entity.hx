class Entity {
	...
	/** Grid Y coordinate **/
    public var cy = 0;
	/** Sub-grid X coordinate (from -0.5 to 0.5) **/
    public var xr = 0.0;
	/** Sub-grid Y coordinate (from -0.5 to 0.5) **/
    public var yr = 0.0;
	...
	public var wid(default,set) : Float = Const.GRID_X;
	...
	public var hei(default,set) : Float = Const.GRID_Y;
	...
	public var sprX(get,never) : Float;
		inline function get_sprX() {
			return interpolateSprPos
				? M.lerp( lastFixedUpdateX, (cx+xr)*Const.GRID_X, game.getFixedUpdateAccuRatio() )
				: (cx+xr)*Const.GRID_X;
		}
	...
	public var sprY(get,never) : Float;
		inline function get_sprY() {
			return interpolateSprPos
				? M.lerp( lastFixedUpdateY, (cy+yr)*Const.GRID_Y, game.getFixedUpdateAccuRatio() )
				: (cy+yr)*Const.GRID_Y;
		}
	...
	public var attachX(get,never) : Float; inline function get_attachX() return (cx+xr)*Const.GRID_X;
	/** Entity attach Y pixel coordinate **/
	public var attachY(get,never) : Float; inline function get_attachY() return (cy+yr)*Const.GRID_Y;
	...
	**/
	...
	public function setPosPixel(x:Float, y:Float) {
		cx = Std.int(x/Const.GRID_X);
		cy = Std.int(y/Const.GRID_Y);
		xr = (x-cx*Const.GRID_X)/Const.GRID_X;
		yr = (y-cy*Const.GRID_Y)/Const.GRID_Y;
		onPosManuallyChangedBoth();
	}
	...
	function onPosManuallyChangedBoth() {
		if( M.distSqr(attachX,attachY,prevFrameAttachX,prevFrameAttachY) > Const.GRID_X*Const.GRID_Y*4 ) {
	...
	function onPosManuallyChangedX() {
		if( M.fabs(attachX-prevFrameAttachX) > Const.GRID_X*2 )
			prevFrameAttachX = attachX;
	...
	function onPosManuallyChangedY() {
		if( M.fabs(attachY-prevFrameAttachY) > Const.GRID_Y*2 )
			prevFrameAttachY = attachY;
