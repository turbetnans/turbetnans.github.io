...
class Fx extends GameChildProcess {
	...
	public inline function markerCase(cx:Int, cy:Int, sec=3.0, c:Col=Pink) {
		#if debug
		var p = allocMain_add(D.tiles.fxCircle15, (cx+0.5)*Const.GRID_X, (cy+0.5)*Const.GRID_Y);
		p.setFadeS(1, 0, 0.06);
		p.colorize(c);
		p.lifeS = sec;

		var p = allocMain_add(D.tiles.pixel, (cx+0.5)*Const.GRID_X, (cy+0.5)*Const.GRID_Y);
		p.setFadeS(1, 0, 0.06);
		p.colorize(c);
		p.setScale(2);
		p.lifeS = sec;
	...
	inline function collides(p:HParticle, offX=0., offY=0.) {
		return level.hasCollision( Std.int((p.x+offX)/Const.GRID_X), Std.int((p.y+offY)/Const.GRID_Y) );
	}
