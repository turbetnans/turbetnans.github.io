...
class LPoint {
	...
	public var levelX(get,set) : Float;
		inline function get_levelX() return (cx+xr)*Const.GRID_X;
	...
	public var levelY(get,set) : Float;
		inline function get_levelY() return (cy+yr)*Const.GRID_Y;
	...
	inline function setLevelPixelX(x:Float) {
		...
		this.xr = ( x % Const.GRID_X ) / Const.GRID_X;
		return this;
	}

	inline function setLevelPixelY(y:Float) {
		cy = Std.int(y/Const.GRID_Y);
		this.yr = ( y % Const.GRID_Y ) / Const.GRID_Y;
		return this;
	}
